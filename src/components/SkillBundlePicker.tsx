...
export function SkillBundlePicker({
  ...
  const filtered = skillPacks.filter(
    (pack) =>
      pack.name.toLowerCase().includes(q) ||
      (pack.description?.toLowerCase().includes(q) ?? false) ||
      pack.skills.some((s) => s.toLowerCase().includes(q))
  );
  ...
  useEffect(() => {
    ...
    const items = listRef.current.querySelectorAll<HTMLElement>("[data-bundle-item]");
  ...
  const apply = useCallback(
    (pack: SkillPack) => {
      onApplySkillPack?.(pack);
      close();
      triggerRef.current?.focus();
    },
    [onApplySkillPack, close]
  );
  ...
  return (
    <div className="flex items-center gap-1.5 shrink-0">
      <button
        ...
        title="Apply a saved skill bundle"
      >
        <span>Bundles</span>
        <span className="text-[10px] text-gray-500 tabular-nums">({skillPacks.length})</span>
        <svg
          width="10"
          height="10"
          viewBox="0 0 24 24"
            ...
            >
              <div className="px-3 pt-3 pb-2 border-b border-border-subtle/50 shrink-0">
                ...
                <p className="text-[10px] text-gray-500 mt-1.5 px-0.5">
                  {searchQuery
                    ? `${filtered.length} of ${skillPacks.length}`
                    : `${skillPacks.length} skill bundle${skillPacks.length !== 1 ? "s" : ""}`}
                </p>
              </div>
                    ...
                    >
                      ...
                      {pack.skills.length > 0 && (
                        <p className="text-[10px] text-gray-600 mt-1 truncate">
                          ...
                          {pack.strict === false ? " · allow additional skills" : " · strict"}
                        </p>
                      )}
                    </button>
                  ))
                ) : (
                  <p className="text-sm text-gray-500 px-3 py-4 text-center">No matching bundles</p>
                )}
              </div>
            </div>
          </>,
