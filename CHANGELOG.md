...
- **Playbook categories**: playbooks now carry an optional `category` field and are grouped under category headers in both the Playbooks picker (under the message box) and the Settings → Playbooks editor, so the list stays scannable as it grows. Existing playbooks without a category auto-group by name prefix (e.g. `Okta …`, `Duo …`, `Workday …`), falling back to "Uncategorized"; search and keyboard navigation are unchanged.
...
- Removed a duplicate audit-log viewer (and duplicate `GET /api/audit` route) that had been added under Settings → System; the dedicated Audit log tab is now the single source.
